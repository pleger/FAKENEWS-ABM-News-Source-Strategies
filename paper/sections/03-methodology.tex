...
The temporal order within period $t$ is:
\begin{enumerate}
\item scheduled strategies are activated or restored, and each source draws one binary publication state $Z_{st}\sim\mathrm{Bernoulli}(p_s)$;
\item pending label-derived evidence scheduled for $t$ is delivered and updates retained evidence;
\item active users evaluate known sources and select at most one, after which selection, false-publication and unique-reposter outcomes are recorded;
\item each receiver observes the strongest current recommendation among directed contacts and discovers an unknown recommended source before coverage is sampled; and
\item a covered recommendation is classified and, when its configured effect is non-zero, schedules evidence for $t+1+d$.
\end{enumerate}
...
\begin{table}[ht]
\centering
\caption{Staged primary experimental design. Run files are stochastic replications; common-seed treatment and control files are intentionally dependent. The 360 diagnostic re-executions are reported separately and are excluded from primary inferential comparisons.}
\label{tab:design}
\small
\begin{tabular}{lrrrp{5.2cm}}
\toprule RQ & Conditions & Runs per condition & Runs & Main factors \\
\midrule
RQ1 & 6 & 30 & 180 & Five source strategies and control \\
RQ2 & 19 & 30 & 570 & Strategy, start period and duration \\
RQ3 & 92 & 30 & 2,760 & Reach, strategy, recommendation policy, label quality and receiver scale \\
RQ4 & 306 & 10--15 & 3,180 & Memory/contact grid, Morris screen, activity and topology \\
\midrule Primary total & 423 & -- & 6,690 & Original and major-revision suites \\
RQ3 diagnostics & 12 & 30 & 360 & Mechanism audit only; excluded from primary inference \\
\bottomrule
\end{tabular}
\end{table}
